...
class Market: 
    ...
    def get_atm_chain(self) -> ATMChainQuote: 
        ...
        for e in self.option_expiries:
            ...
            mapped_put_delta = {q.instrument_name: q.sensitivities.delta for q in quotes 
                                if q.instrument_name in opt_name 
                                and q.instrument_name in self.put_options_name}
            mapped_call_delta = {q.instrument_name: q.sensitivities.call_delta 
                                 for q in quotes
                                 if q.instrument_name in opt_name 
            ...
            put_diff_to_atm = [abs(abs(mapped_put_delta[k])-0.5) 
                               for k in list(mapped_put_delta.keys())]
            call_name = [k for f in list(mapped_call_delta.keys())
                         if abs(mapped_call_delta[k]-0.5) == min(call_diff_to_atm)][0]
            put_name = [k for f in list(mapped_call_delta.keys())
                         if abs(mapped_call_delta[k]-0.5) == min(call_diff_to_atm)][0]
