...
class PerpetualFuture(Instrument): 
    ...
    def get_margin(self, size:float) -> Margin: 
        if self.risk_factor.base_currency.code == 'ETH':
            im_rate = 0.02 + abs(size)*0.004/100
            mm_rate = 0.01 + abs(size)*0.004/100
        if self.risk_factor.base_currency.code == 'BTC':
...
class Spot(Instrument): 
    ...
    def get_margin(self, size: float) -> Margin:
        return Margin(self.order_book.quote_currency,0,0)
    
    def get_trading_fees(self) -> TradingFee: 
        return TradingFee(self.order_book.quote_currency, 0)  
